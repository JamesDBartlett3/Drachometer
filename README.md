...
- Detects an expired OAuth session (the "Failed to authenticate" error Claude Code's first run of the day can hit) and opens a visible, interactive `claude` window for you to sign back in; the timestamp file is only updated once a ping actually succeeds, so a failed ping keeps retrying instead of going quiet for 5 hours.
